...
vendor certificates that have not previously signed any second stage bootlaoder
or EFI binaries.
...
All package versions are displayed in the build log. The included Makefile and
Dockerfile are the same as what we use to build our shim. A new shim can be built
using the `make build` command.
